...
abstract class Classifier {
  ...
  final String _labelsFileName = 'assets/tflite/label_map.pbtxt';
  ...
  Category predict(Image image) {
    ...
    print('asdasd_ ${image.exif}');

    final runs = DateTime.now().millisecondsSinceEpoch;

    var map = <int, Object>{};
    map[0] = _outputBuffer.getBuffer();
    var inputTensors = interpreter.getInputTensors();
    var outputTensors = interpreter.getOutputTensors();
    var inputs = [_inputImage.buffer];

    for (int i = 0; i < inputs.length; i++) {
      print('test d ${inputs[i].lengthInBytes}');
      inputTensors.elementAt(i).setTo(inputs[i]);
    }

    log.log("test a ${inputTensors}");
    log.log("test b ${outputTensors}");
    log.log("test c ${_outputBuffer.getBuffer()}");

    try {
      for (var i = 0; i < outputTensors.length; i++) {
        outputTensors[i].copyTo(map[i]!);
      }
    } catch (e) {
      print("test $e");
    }
